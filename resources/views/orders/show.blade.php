<x-layouts::main-content title="Order #{{ $order->id }}" heading="Order Details" subheading="Reviewing individual transaction #{{ $order->id }}">
    <div class="max-w-5xl mx-auto py-6 space-y-6">
        <div class="p-6 bg-zinc-50 border border-zinc-200 rounded-xl shadow-sm">
            <h3 class="font-bold text-lg mb-4 text-zinc-900">Ordered Items</h3>
            <ul class="space-y-4">
                @foreach($order->items as $item)
                    <li class="flex justify-between items-center border-b pb-4 last:border-0 last:pb-0">
                        <div class="flex items-center gap-4">
                            <div class="size-12 bg-zinc-200 rounded-lg" style="background-color: #{{ $item->color_code }}"></div>
                            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="p-6 bg-zinc-50 border border-zinc-200 rounded-xl shadow-sm">
                ...
                <p class="text-sm text-zinc-600">{{ $order->address }}</p>
            </div>
            <div class="p-6 bg-zinc-50 border border-zinc-200 rounded-xl shadow-sm">
                <h3 class="font-bold mb-2">Payment Details</h3>
                <p class="text-sm text-zinc-600">Method: {{ $order->payment_type }}</p>
